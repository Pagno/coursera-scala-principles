...
object Huffman {
  ...
  // Part 1: Basics
    ...
    def chars(tree: CodeTree): List[Char] = tree match {
		...
		case Fork(left: CodeTree, right: CodeTree, chars: List[Char], weight: Int) => chars
			//Huffman.chars(left) :: Huffman.chars(right) :: List[Char](char)
    } // tree match ...
   ...
   * unchanged.
   */
    def combine(trees: List[CodeTree]) = {
		trees match {
    		//case x:CodeTree::y:CodeTree::xs:List[CodeTree] => {xs}
    		case (x:Leaf)::(y:Leaf)::xs => makeCodeTree(x,y)::xs
			case _ => ()
   ...
   */
    def createCodeTree(chars: List[Char]): CodeTree = {
		...
		def f(x: CodeTree,y: CodeTree) = {
			/*y match{
				case Leaf(char: Char, weight: Int) => {
					return makeCodeTree(x,y)
				}
   ...
   */
    def decode(trees: CodeTree, bits: List[Bit]): List[Char] = {
		...
		def iterate(tree: CodeTree, bits: List[Bit],charsList: List[Char]): List[Char] = tree match {
				case Leaf(char: Char, weight: Int) => {
					...
					if(bits.length==0){
						...
						return iterate(trees, bits, charsList:::List[Char](char))
					}
				}
				case Fork(left: CodeTree, right: CodeTree, chars: List[Char], weight: Int) => {
   ...
   */
    def encode(trees: CodeTree)(text: List[Char]): List[Bit] = {
    	
		def iterate(tree: CodeTree, c: Char, result: List[Bit]): List[Bit] = tree match {
			case Leaf(char: Char, weight: Int) => {
				if(char.equals(c)){
			...
			case Fork(left: CodeTree, right: CodeTree, chars: List[Char], weight: Int) => {
				if(chars.contains(c)){
					return iterate(left,c,0::result) ::: iterate(right,c,1::result)
				}else{
					return List[Bit]()
				}
			}
		...
		text.map(x => iterate(trees,x,List[Bit]())).flatten
    }
